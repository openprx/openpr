...
| Decimal-safe amount fields | `apps/api/src/forms/decimal.rs`, `apps/api/src/forms/values.rs`, `apps/api/src/forms/projections.rs` | Amount field input, indexed projections, aggregate reads | `cargo test -p api forms::`, `scripts/smoke-restaurant-ordering.sh` | `已测试` |
...
| Business events and delivery ledger | `apps/api/src/events/`, `apps/worker/src/main.rs`, `migrations/0031_business_events_outbox_inbox.sql` | Business events, outbox, inbox, connector receipts | `scripts/smoke-form-events-outbox.sh`, `scripts/smoke-webhook-generic-consumer.sh` | `已测试` |
| Connectors, webhooks, and print | `apps/api/src/routes/connector.rs`, `apps/api/src/routes/webhook.rs`, `apps/worker/src/main.rs`, `migrations/0032_print_device_connector_kinds.sql` | Passive webhook consumers, print/device connectors, receipt API | `scripts/smoke-print-connector.sh`, `scripts/smoke-openpr-webhook-routing.sh` | `已测试` |
...
| MCP business surface | `apps/mcp-server/src/tools/forms.rs`, `apps/mcp-server/src/tools/plugins.rs`, `apps/mcp-server/src/tools/connectors.rs`, `apps/mcp-server/src/tools/scenario_templates.rs` | 106-tool MCP registry over HTTP, stdio, and SSE | `skills/openpr-mcp/scripts/validate-mcp.sh`, `skills/openpr-mcp/scripts/mcp-regression.py` | `已测试` |
...
| Restaurant reference scenario | `scripts/bootstrap-restaurant-demo.sh`, `apps/api/src/routes/project.rs`, `frontend/scripts/smoke-restaurant-ordering.mjs` | Menu, SKU, table, order, order lines, table change, print jobs, report aggregate | `scripts/smoke-restaurant-ordering.sh`, `scripts/smoke-restaurant-demo-bootstrap-mcp-http.sh` | `已测试` |
