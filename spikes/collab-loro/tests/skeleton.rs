use collab_loro_spike::{InputError, InputLimits, IsolationKind, REQUIRED_RUST_ISOLATION_KIND, metadata};

#[test]
fn locks_candidate_metadata() {
    let metadata = metadata();
    assert_eq!(metadata.candidate, "loro");
    assert_eq!(metadata.rust_engine, "loro");
    assert_eq!(metadata.rust_engine_version, "1.13.9");
}
