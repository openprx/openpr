...
const factory = () => new YjsScenarioAdapter();

describe("YjsScenarioAdapter against the shared corpus runner (real yjs@13.6.32)", () => {
  test("same_text_range_edit: concurrent inserts into the same range converge to an identical semantic hash", async () => {
    const result = await runSameTextRangeEditCase(factory, "yjs-corpus-seed-1");
  ...
  test("concurrent_block_move: a child moved to two different parents converges to a single parent with no cycle", async () => {
    const result = await runConcurrentBlockMoveCase(factory, "yjs-corpus-seed-2");
    expect(result.status).toBe("passed");

    // Independently re-derive the invariant from the real Y.Map tree, not just the runner's hash comparison.
    ...
    const other = new YjsScenarioAdapter();
  ...
  test("duplicate_out_of_order_partial_batch: re-importing the same update is idempotent", async () => {
    const result = await runDuplicateApplyIdempotentCase(factory, "yjs-corpus-seed-3");
    expect(result.status).toBe("passed");
  });

  test("adversarial_update_limits: a truncated update is rejected and leaves head/frontier unchanged", async () => {
    const result = await runCorruptUpdateRejectedCase(factory, "yjs-corpus-seed-4");
    expect(result.status).toBe("passed");
