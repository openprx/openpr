...
for minor in range(3,10):
 ...
 status_accepted=is_accepted_contract_status(contract_status)
...
mutations={
 ...
 "unknown_contract_status":{"value":"accepted_typo","red":not is_accepted_contract_status("accepted_typo")},
...
result={"schema_version":"sylvode.flow.prior-receipts-manifest.v2","release":"1.0.0","acceptance_authority":"contract_status_via_scripts/lib/flow_contract_status.py","releases":releases,"excluded":exclusions,"exclusion_coverage":coverage,"files":files,"mutations":mutations,"executed_count":len(releases)+len(files)+1+len(mutations),"passed":passed,"generated_at":dt.datetime.now(dt.timezone.utc).isoformat()}
