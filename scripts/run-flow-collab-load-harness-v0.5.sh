...
PG_LOGGING_COLLECTOR="$(psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -Atc 'show logging_collector' 2>/dev/null || true)"
PG_LOG_DESTINATION="$(psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -Atc 'show log_destination' 2>/dev/null || true)"
PG_LOG_LINE_PREFIX="$(psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -Atc 'show log_line_prefix' 2>/dev/null || true)"
...
[[ "$PG_LOGGING_COLLECTOR" == on ]] || add_preflight_violation "flow-test-pg logging_collector must be on; journald container logs are rate-limited"
[[ ",$PG_LOG_DESTINATION," == *,stderr,* ]] || add_preflight_violation "flow-test-pg log_destination must include stderr"
[[ "$PG_LOG_LINE_PREFIX" == '%m [%p] ' ]] || add_preflight_violation "flow-test-pg log_line_prefix must be exactly %m [%p] "
