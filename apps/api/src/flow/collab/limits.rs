...
/// One frozen caller-visible rejection kind and the numeric ceiling its wire `details.limit`
/// carries. This registry is deliberately separate from `FlowLimitsV1`: the latter omits
/// transport-only/internal-executor kinds, while W1-b requires the complete 32-kind error wire
/// vocabulary to stay enumerable and testable as one closed set.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct LimitKindDescriptor {
    pub kind: &'static str,
    pub limit: u64,
}

/// The complete v0.4 fixed `limit_kind` set from `contracts/limits-v1.md` (unset and v0.5-only
/// rows excluded). Real enforcement remains at each owning call site; this registry supplies the
/// exhaustive wire producer fixture that proves none of those real kinds is serialization-dead.
#[must_use]
pub const fn limit_kind_descriptors() -> [LimitKindDescriptor; 32] {
    [
        LimitKindDescriptor {
            kind: "update_bytes",
            limit: UPDATE_BYTES_MAX,
        },
        LimitKindDescriptor {
            kind: "websocket_frame_bytes",
            limit: WEBSOCKET_FRAME_BYTES_MAX as u64,
        },
        LimitKindDescriptor {
            kind: "presence_payload_bytes",
            limit: PRESENCE_PAYLOAD_BYTES_MAX,
        },
        LimitKindDescriptor {
            kind: "presence_ttl_seconds",
            limit: PRESENCE_TTL_SECONDS_MAX as u64,
        },
        LimitKindDescriptor {
            kind: "bootstrap_decoded_bytes",
            limit: BOOTSTRAP_DECODED_BYTES_MAX,
        },
        LimitKindDescriptor {
            kind: "bootstrap_response_bytes",
            limit: BOOTSTRAP_RESPONSE_BYTES_MAX,
        },
        LimitKindDescriptor {
            kind: "tree_depth",
            limit: TREE_DEPTH_MAX,
        },
        LimitKindDescriptor {
            kind: "container_count",
            limit: CONTAINER_COUNT_MAX,
        },
        LimitKindDescriptor {
            kind: "document_block_count",
            limit: DOCUMENT_BLOCK_COUNT_MAX,
        },
        LimitKindDescriptor {
            kind: "text_block_chars",
            limit: TEXT_BLOCK_CHARS_MAX,
        },
        LimitKindDescriptor {
            kind: "document_text_chars",
            limit: DOCUMENT_TEXT_CHARS_MAX,
        },
        LimitKindDescriptor {
            kind: "semantic_patch_operations",
            limit: SEMANTIC_PATCH_OPERATIONS_MAX,
        },
        LimitKindDescriptor {
            kind: "semantic_patch_bytes",
            limit: SEMANTIC_PATCH_JSON_BYTES_MAX,
        },
        LimitKindDescriptor {
            kind: "decode_apply_cpu_ms",
            limit: DECODE_APPLY_CPU_MS_MAX,
        },
        LimitKindDescriptor {
            kind: "decode_apply_wall_ms",
            limit: DECODE_APPLY_WALL_MS_MAX,
        },
        LimitKindDescriptor {
            kind: "isolated_apply_memory_bytes",
            limit: ISOLATED_APPLY_MEMORY_BYTES_MAX,
        },
        LimitKindDescriptor {
            kind: "open_documents",
            limit: OPEN_DOCUMENTS_PER_CONNECTION_MAX,
        },
        LimitKindDescriptor {
            kind: "user_connections",
            limit: CONNECTIONS_PER_USER_MAX,
        },
        LimitKindDescriptor {
            kind: "document_connections",
            limit: CONNECTIONS_PER_DOCUMENT_MAX,
        },
        LimitKindDescriptor {
            kind: "workspace_connections",
            limit: CONNECTIONS_PER_WORKSPACE_MAX,
        },
        LimitKindDescriptor {
            kind: "presence_entries_per_connection",
            limit: PRESENCE_ENTRIES_PER_CONNECTION_MAX as u64,
        },
        LimitKindDescriptor {
            kind: "presence_entries_per_document",
            limit: PRESENCE_ENTRIES_PER_DOCUMENT_MAX as u64,
        },
        LimitKindDescriptor {
            kind: "frame_rate",
            limit: FRAMES_PER_CONNECTION_PER_SECOND,
        },
        LimitKindDescriptor {
            kind: "update_rate",
            limit: UPDATES_PER_CONNECTION_PER_SECOND,
        },
        LimitKindDescriptor {
            kind: "slow_consumer_queue_frames",
            limit: SLOW_CONSUMER_QUEUE_FRAMES_MAX,
        },
        LimitKindDescriptor {
            kind: "slow_consumer_queue_bytes",
            limit: SLOW_CONSUMER_QUEUE_BYTES_MAX,
        },
        LimitKindDescriptor {
            kind: "page_size",
        ...
        LimitKindDescriptor {
            kind: "scan_budget",
            limit: AUTHORIZED_SCAN_ROWS_MAX,
        },
        LimitKindDescriptor {
            kind: "import_archive_bytes",
            limit: IMPORT_ARCHIVE_BYTES_MAX,
        },
        LimitKindDescriptor {
            kind: "import_expanded_bytes",
            limit: IMPORT_EXPANDED_BYTES_MAX,
        },
        LimitKindDescriptor {
            kind: "import_entry_count",
            limit: IMPORT_ENTRY_COUNT_MAX,
        },
        LimitKindDescriptor {
            kind: "import_compression_ratio",
            limit: IMPORT_COMPRESSION_RATIO_MAX,
        },
    ]
}
...
mod tests {
    ...
    use super::{effective_limits, limit_kind_descriptors};
    ...
    #[tokio::test]
    async fn all_32_limit_kinds_are_observable_on_the_rest_wire_fixture() {
        let descriptors = limit_kind_descriptors();
        let unique: HashSet<&str> = descriptors.iter().map(|descriptor| descriptor.kind).collect();
        assert_eq!(descriptors.len(), 32);
        assert_eq!(unique.len(), 32, "every descriptor must name a distinct wire kind");

        for descriptor in descriptors {
            let response = ApiError::limit_exceeded(
                "controlled limit producer fixture",
                descriptor.kind,
                Some(json!(descriptor.limit)),
                Some(json!(descriptor.limit.saturating_add(1))),
                None,
            )
            .into_response();
            assert_eq!(response.status(), axum::http::StatusCode::OK);
            let bytes = to_bytes(response.into_body(), 32_768).await.expect("body reads");
            let body: serde_json::Value = serde_json::from_slice(&bytes).expect("body is JSON");
            assert_eq!(body["error_code"], "limit_exceeded");
            assert_eq!(body["details"]["limit_kind"], descriptor.kind);
            assert_eq!(body["details"]["limit"], descriptor.limit);
        }
    }
}
