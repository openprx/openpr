...
impl CollabRuntime {
    ...
    pub fn begin_workspace_drain(&self, workspace_id: Uuid, retry_after_ms: u64) -> WorkspaceDrainGuard<'_> {
        {
            ...
            entries
                ...
                .and_modify(|entry| {
                    ...
                    entry.retry_after_ms = entry.retry_after_ms.max(retry_after_ms);
                })
                .or_insert(DrainEntry {
                    holders: 1,
                    retry_after_ms,
                });
        }
    ...
    pub fn workspace_drain_retry_after_ms(&self, workspace_id: Uuid) -> Option<u64> {
        self.drains
            ...
            .lock()
            .get(&workspace_id)
            .map(|entry| entry.retry_after_ms)
    }
    ...
    pub fn ensure_workspace_accepting(&self, workspace_id: Uuid) -> Result<(), ApiError> {
        let Some(retry_after_ms) = self.workspace_drain_retry_after_ms(workspace_id) else {
            return Ok(());
        };
