...
async fn load_user_trust_scores(
    ...
    Ok(
        UserTrustScoreRow::find_by_statement(Statement::from_sql_and_values(DbBackend::Postgres, sql, values))
            .all(&state.db)
            .await?,
    )
}

fn build_user_trust_payload(target_user_id: Uuid, rows: Vec<UserTrustScoreRow>) -> Result<serde_json::Value, ApiError> {
    if rows.is_empty() {
    ...
    let user_type = rows
        .first()
        .ok_or_else(|| ApiError::NotFound("trust score not found".to_string()))?
        .user_type
        .clone();
...
pub async fn get_user_trust_by_domain(
    ...
    let rows = load_user_trust_scores(&state, user_id, target_user_id, query.project_id, Some(domain)).await?;
    let payload = build_user_trust_payload(target_user_id, rows)?;
    Ok(ApiResponse::success(payload))
}
...
pub async fn list_user_trust_history(
    ...
    let where_sql = where_parts.join(" AND ");
    let count_sql = format!("SELECT COUNT(*)::bigint AS count FROM trust_score_logs t WHERE {where_sql}");
    let total = CountRow::find_by_statement(Statement::from_sql_and_values(
        DbBackend::Postgres,
        count_sql,
        values.clone(),
