...
async fn main() -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .route(
            ...
            get(routes::flow::get_flow_projection_lag).route_layer(axum_middleware::from_fn_with_state(
                auth_state.clone(),
                middleware::bot_auth::bot_or_user_auth_middleware,
            )),
        )
        ...
        .route(
            "/api/v1/flow/objects/{object_id}/diff",
            get(routes::flow::get_flow_object_diff).route_layer(axum_middleware::from_fn_with_state(
                auth_state.clone(),
                middleware::bot_auth::bot_or_user_auth_middleware,
            )),
        )
        ...
        // The custom span preserves the tower-http fields but records only the URI path. Query
        // strings carry both secrets (collab tickets) and user content (search/filter terms), so
        // sanitization is global rather than an allowlist of routes that will drift.
